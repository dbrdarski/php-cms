...
class Validator{
    ...
    private $value;
    private statitc $faliures = array();
    private statitc $validators = array(
        'numeric'       => "/^[0-9]*$/",
        'alpha'         => "/^[a-zA-Z0-9_]*$/",
    ...
    {       
        $pattern = $validators[$method];
        $message = array_shift($arguments);
        $test = preg_match($pattern, $this->value);
        if(!$test){
            $faliures[$pattern] = $message ? $message : $pattern;
        }
        return $this;
    }
    public function pattern($pattern, $name, $message = null){
        $test = preg_match($pattern, $this->value);
        if(!$test){
            $faliures[$name] = $message ? $message : $name;
        }
        return $this;
    }
}